...
class Client extends Controller
{
    ...
    public function formAddConta(Request $request){
        ...
        if(empty($nome)){ return 'Campo nome da entidade vazio.';}
        if(empty($email)){ return 'Campo endereço de email vazio.';}
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)){ return 'O campo endereço de email inválido. Introduza um email válido.'; }
        if(empty($contacto)){ return 'Campo contacto vazio.';}
        if(empty($morada)){ return 'Campo morada vazio.';}
        if(empty($codigo_postal)){ return 'Campo código postal vazio.';}
        if(empty($localidade)){ return 'Campo localidade vazio.';}
        if(empty($pais)){ return 'Campo país vazio.';}
        if(empty($nif)){ return 'Campo NIF vazio.';}
