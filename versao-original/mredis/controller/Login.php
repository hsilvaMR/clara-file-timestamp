...
class Login extends Controller
{
	...
	public function formLogin(Request $request){
		...
		if (!filter_var($email, FILTER_VALIDATE_EMAIL)){ return 'O campo endereço de email inválido. Introduza um endereço de email válido.'; }
		if(empty($password)){ return 'Campo palavra-passe vazio.';}
	    ...
	    if(isset($client->id)){
	    	...
	    	if(!Hash::check($password, $client->password)){ return 'Dados incorretos. Email ou password incorretos.'; }
	...
	public function formRegister(Request $request){
		...
		if (!filter_var($email, FILTER_VALIDATE_EMAIL)){ return 'O campo endereço de email inválido. Introduza um endereço de email válido.'; }
		if(empty($password)){ return 'Campo palavra-passe vazio.';}
		if(strlen($password) < 8){ return 'Campo palavra-passe é composto no mínimo por 8 caracteres.';}
